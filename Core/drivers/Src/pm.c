...
PmStates pmUpdateState() {
  ...
  batteryLowTime = osKernelGetTickCount() - batteryLowTimeStamp;
  ...
  while (1) {
    ...
    switch (pmState) {
      ...
      case charging:
          ...
          chargeLevel = pmBatteryChargeFromVoltage(pmGetBatteryVoltage()) / 10.0f;
          ledseqSetChargeLevel(chargeLevel);
        break;
      case lowPower:
          if (tickCount - batteryCriticalLowTimeStamp > PM_BAT_CRITICAL_LOW_TIMEOUT)
            pmSystemShutdown();
        break;
      case battery:
