...
// TODO: decide which are essential
...
static inline void estimatorEnqueueGyroscope(const gyroscopeMeasurement_t *gyroscope) {
  measurement_t m;
  m.type = MeasurementTypeGyroscope;
  m.data.gyroscope = *gyroscope;
  estimatorEnqueue(&m);
}

static inline void estimatorEnqueueAcceleration(const accelerationMeasurement_t *acceleration) {
  measurement_t m;
  m.type = MeasurementTypeAcceleration;
  m.data.acceleration = *acceleration;
  estimatorEnqueue(&m);
}
